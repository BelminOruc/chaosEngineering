...
def read_sndlib_topology(file):
    ...
    with (open('sndlibcontent/' + file) as file):
        ...
        for idx, link in enumerate(links):
            ...
            last_module = additional_modules[0]  # Get the last module
            ...
            #cost = link.getElementsByTagName("cost")[idx].childNodes[0].data
            ...
            graph.add_edge(source.childNodes[0].data, target.childNodes[0].data,
                           id=link.getAttribute("id"), capacity=capacity, cost=cost, length=length, index=idx)
